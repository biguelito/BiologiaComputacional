...
def needleman_wunsch(s1 : str, s2 : str, match_value : int, mismatch_value : int, gap_value : int):
    ...
    matrix[:,0] = [0 for i in range(size_s1+1)]
    matrix[0,:] = [0 for i in range(size_s2+1)]
...
def log(actual_pos, next_pos, actual : int, path : str, back_score : int, print_path=False):
    print(f"From {actual} {actual_pos} moves {path} to {back_score} {next_pos}") if print_path else None
    return

def get_max(list_data : list, data : str, last=False):
    return len(list_data) - 1 - list_data[::-1].index(data) if last else list_data.index(data)

def get_max_position(matrix : list, s1 : str, s2 : str, last=False):
    line = [i[-1] for i in matrix]
    column = list(matrix[-1])
    max_line = max(line)
    max_column = max(column)
    bigger_on_line = max_line > max_column   

    if (bigger_on_line):
        line_position = get_max(line, max_line, last)
        column_position = len(s2)
    else:
        line_position = len(s1)
        column_position = get_max(column, max_column, last)

    return (line_position, column_position)

def complete_semi_global(original_s1 : str, original_s2 : str, aligned_s1 : str, aligned_s2 : str, line_pos, column_pos):
    rest_s1 = original_s1[line_pos:]
    rest_s2 = original_s2[column_pos:]
    complete_s1 = aligned_s1 + rest_s1
    complete_s2 = aligned_s2 + rest_s2
    if len(complete_s1) > len(complete_s2):
        diff = len(complete_s1) - len(complete_s2)
        complete_s2 += "-" * diff
    elif len(complete_s1) < len(complete_s2):
        diff = len(complete_s2) - len(complete_s1)
        complete_s1 += "-" * diff
...
def backtracking(s1 : str, s2 : str, matrix : list, match_value : int, mismatch_value : int, gap_value : int, print_path=False):
    ...
    initial_line_position, initial_column_position = get_max_position(matrix, s1, s2)
    line_position = int(initial_line_position)
    column_position = int(initial_column_position)
    ...
    while line_position > 0 or column_position > 0:
        ...
        if column_position == 0:
            ...
            log(actual_pos, next_pos, actual, path, back_score, print_path)
            continue
    ...
    return complete_semi_global(s1, s2, aligned_s1, aligned_s2, initial_line_position, initial_column_position)

    return aligned_s1, aligned_s2

def execute_semi_global_alignment(s1 : str, s2 : str, match_value : int, mismatch_value, gap_value : int, print_path=False):
